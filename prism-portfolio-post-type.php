...
class Prism_Portfolio_Post_Type {
	...
	function Prism_Portfolio_Post_Type() {	
		...
		add_filter('manage_edit-prism_portfolio_columns', array(&$this,'edit_columns'));
		...
		add_filter('manage_edit-prism_portfolio_sortable_columns', array(&$this,'sortable_columns'));
		...
		add_action('wp_ajax_prism_set_thumbnail', array(&$this,'set_thumbnail_callback'));	
...
function _mu_deactivate($permastruct, $ep_mask=EP_NONE) {
	...
	function display_columns($portfolio_columns, $post_id){
		...
		switch ( $portfolio_columns ) {
			...
			case "thumbnail":
				...
				case "prism_portfolio_category":
			
					$taxonomies = get_the_terms( $post_id, 'prism_portfolio_category' ) ; 
					if ( !empty( $taxonomies ) ) {
						$out = array();
						foreach ( $taxonomies as $t ) {  
							$out[] = sprintf( '<a href="%s">%s</a>',
								esc_url( add_query_arg( array( 'post_type' => $post->post_type, 'prism_portfolio_category' => $t->slug ), 'edit.php' ) ),
								esc_html( sanitize_term_field( 'name', $t->name, $t->term_id, 'prism_portfolio_category', 'display' ) )
							);
						}
						echo join( ', ', $out );
			...
			break;
				...
				case "prism_portfolio_tag":
				
				$taxonomies = get_the_terms( $post_id, 'prism_portfolio_tag' ) ; 
					if ( !empty( $taxonomies ) ) {
						$out = array();
						foreach ( $taxonomies as $t ) {  
							$out[] = sprintf( '<a href="%s">%s</a>',
								esc_url( add_query_arg( array( 'post_type' => $post->post_type, 'prism_portfolio_tag' => $t->slug ), 'edit.php' ) ),
								esc_html( sanitize_term_field( 'name', $t->name, $t->term_id, 'prism_portfolio_tag', 'display' ) )
							);
						}
						echo join( ', ', $out );
				...
				case "prism_portfolio_featured":
				
				$taxonomies = get_the_terms( $post_id, 'prism_portfolio_featured' ) ; 
					if ( !empty( $taxonomies ) ) {
						$out = array();
						foreach ( $taxonomies as $t ) {  
							$out[] = sprintf( '<a href="%s">%s</a>',
	...
	function add_quick_edit($column_name, $post_type) {
		if ( $post_type != 'prism_portfolio' || $column_name != 'prism_portfolio_featured') return;
		
		?>
		<fieldset class="inline-edit-col-left">
		<div class="inline-edit-col">
			<span class="title"><?php _e('Featured Status');?></span><br/>
			<input type="hidden" name="prism_portfolio_featured_nonce" id="prism_portfolio_featured_nonce" value="" />
	...
	function quick_edit_javascript() {
		...
		if (($current_screen->id != 'edit-prism_portfolio') || ($current_screen->post_type != 'prism_portfolio')) return; 
	...
	function expand_quick_edit_link($actions, $post) {
		global $current_screen; 
		if (($current_screen->id != 'edit-prism_portfolio') || ($current_screen->post_type != 'prism_portfolio')) return $actions; 
	 
		$nonce = wp_create_nonce( 'prism_featured_nonce_' . $post->ID );

		$featured = wp_get_object_terms($post->ID, 'prism_portfolio_featured'); 
	...
	*/
	function save_taxonomy_data($post_id,$post) {  
		...
		if (isset($_POST['prism_portfolio_featured_tax']) ) { 
			...
			if ($status) {
				wp_set_object_terms( $post_id, $status, 'prism_portfolio_featured' );
			} else { 
				wp_set_object_terms( $post_id, 'normal', 'prism_portfolio_featured' );
			}
		}
	...
	*/
	function sortable_columns( $columns ) {
		$columns['prism_portfolio_featured'] = 'portfolio_featured';
		//to do: $columns['prism_portfolio_category'] = 'portfolio_category';
		return $columns;
	}
	 ...
	 */
	 function portfolio_featured_clauses( $clauses, $wp_query ) {
		...
		if ( isset( $wp_query->query['orderby'] ) && 'prism_portfolio_featured' == $wp_query->query['orderby'] ) {
...
SQL;
			...
			$clauses['where'] .= " AND (taxonomy = 'prism_portfolio_featured' OR taxonomy IS NULL)";
			$clauses['groupby'] = "object_id";
			$clauses['orderby']  = "GROUP_CONCAT({$wpdb->terms}.name ORDER BY name ASC) ";
			$clauses['orderby'] .= ( 'ASC' == strtoupper( $wp_query->get('order') ) ) ? 'ASC' : 'DESC';
		}
		//sort by portfolio category
		if ( isset( $wp_query->query['orderby'] ) && 'prism_portfolio_category' == $wp_query->query['orderby'] ) {
...
LEFT OUTER JOIN {$wpdb->terms} USING (term_id)
SQL;
			$clauses['where'] .= " AND (taxonomy = 'prism_portfolio_category' OR taxonomy IS NULL)";
			$clauses['groupby'] = "object_id";
			$clauses['orderby']  = "GROUP_CONCAT({$wpdb->terms}.name ORDER BY name ASC) ";
			$clauses['orderby'] .= ( 'ASC' == strtoupper( $wp_query->get('order') ) ) ? 'ASC' : 'DESC';
		}
	...
	function restrict_manage_posts() {
		...
		$filters = array('prism_portfolio_category','prism_portfolio_tag','prism_portfolio_featured');
	...
	function custom_set_thumbnail(){
		wp_enqueue_script('prism-thumbnails',plugins_url( 'js/prism-set-post-thumbnail.js' , __FILE__ ),array('jquery'));
	}
...
*/
global $prism_ppt;
if (class_exists("Prism_Portfolio_Post_Type") && !$prism_ppt) {
    $prism_ppt = new Prism_Portfolio_Post_Type();	
}	
