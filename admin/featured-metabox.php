 ...
 *
 * @class 		Prism_Portfolio_Featured_Tax
...
class Prism_Portfolio_Featured_Tax extends Prism_Portfolio {

	public static function init() {  
		
		//Add Custom Metabox for Featured Taxonomy
		add_action('add_meta_boxes', array(__CLASS__,'featured_metabox'));
	...
	function featured_metabox(){
		add_meta_box('prism_portfolio_featured_tax', __('Featured or Archived Item',"prism_portfolio"), array(__CLASS__,'featured_tax_display'), "prism_portfolio", 'side', 'low');
	}
	...
	function featured_tax_display($post) { ?>
		...
		<?php $terms = wp_get_post_terms( $post->ID, Prism_Portfolio::$featured  );
		$status = null;
