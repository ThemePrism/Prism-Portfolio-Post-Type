...
function prism_gallery_check_attachment_originality()
{
	...
	if( 'upload' == get_current_screen()->base && ! empty($wp_query->posts) )
	{
		...
		?>
			<script type="text/javascript">
				var prism_gallery_originals = <?php echo $originals; ?>,
					prism_gallery_copies = <?php echo $copies; ?>;

				if( null !== prism_gallery_originals)
					jQuery(prism_gallery_originals).addClass("attachment-original");
				
				if( null !== prism_gallery_copies)
					jQuery(prism_gallery_copies).addClass("attachment-copy");
			</script>
		<?php
		}
	}
}
add_action( 'admin_footer', 'prism_gallery_check_attachment_originality' );
 ...
 * with {@link prism_gallery_parse_attachment_data()}
 */
function prism_gallery_get_attachment_data()
{
	...
	if( 1 < count($attachments) && '' != $linkclass && ! in_array($linkto, array('attachment', 'parent_post', 'none')) )
	{
		...
		$rel = ' rel="' . $linkclass . '[' . $prism_portfolio->Gallery->gallery_id . ']"';
	}
	...
	foreach( $attachments as $attachment_id )
	{
		...
		echo prism_gallery_parse_attachment_data( $attachment, $size, $linkto, $linkclass, $imageclass, $rel, $caption, $align );
	}
	
	exit();
}
add_action('wp_ajax_prism_gallery_send_single', 'prism_gallery_get_attachment_data');
...
function prism_gallery_parse_attachment_data( $attachment, $size, $linkto, $linkclass, $imageclass, $rel, $caption, $align )
{
	...
	if( prism_gallery_file_is_displayable_image( get_attached_file($attachment->ID) ) )
	{
	...
	return apply_filters('prism_gallery_parse_attachment_data', $output, $attachment->ID);
}
 ...
 * Soon...
 */
function prism_gallery_caption_shortcode( $output = "", $attr, $content = null)
{
	extract(
		shortcode_atts(
			array(
...
//add_filter('img_caption_shortcode', 'prism_gallery_caption_shortcode', 10, 3);
...
function prism_gallery_edit_attachment()
{
	...
	$tags = wp_get_object_terms( $attachment->ID, PRISM_GALLERY_MEDIA_TAG_NAME );
	...
	do_action('prism_gallery_edit_attachment', $attachment->ID);
?>
...
<?php do_action('prism_gallery_pre_edit_attachment_post_form', $attachment->ID); ?>
...
<?php
	...
	do_action('prism_gallery_edit_attachment_post_form', $attachment->ID);

	exit();
}
add_action('wp_ajax_prism_gallery_edit_attachment', 'prism_gallery_edit_attachment');
 ...
 */
function prism_gallery_copy_attachments_to_post()
{
	global $wpdb;
	...
	if( is_array($attached_ids) )
	{
		...
		{
			prism_gallery_copy_attachment_to_post( $aid, $post_id );
		}
...
add_action('wp_ajax_prism_gallery_copy_attachments_to_post', 'prism_gallery_copy_attachments_to_post');
 ...
 */
function prism_gallery_copy_attachment_to_post( $aid, $post_id )
{
	global $wpdb;
	...
	$attachment->post_title .= apply_filters('prism_gallery_attachment_copy_title_extension', '', $post_id);
	...
	// copy media tags
	$media_tags = wp_get_object_terms(array($aid), PRISM_GALLERY_MEDIA_TAG_NAME);
	$tags = array();
	...
	wp_set_object_terms($attachment_id, $tags, PRISM_GALLERY_MEDIA_TAG_NAME);
 ...
 */
function prism_gallery_copy_all_attachments()
{
	global $wpdb;
	...
	{
		$r = prism_gallery_copy_attachment_to_post( $aid->ID, $to_id );
...
add_action('wp_ajax_prism_gallery_copy_all_attachments', 'prism_gallery_copy_all_attachments');
 ...
 */
function prism_gallery_delete_attachment( $post_id )
{
	global $wpdb;
	...
	do_action('prism_gallery_delete_attachment', $post_id);
 ...
 */
function prism_gallery_cancel_file_deletion_if_attachment_copies( $file )
{
	global $wpdb;

	if( defined('PRISM_GALLERY_SKIP_DELETE_CANCEL') && true === PRISM_GALLERY_SKIP_DELETE_CANCEL )
		return $file;
	...
	if( is_array($this_copies) && ! empty($this_copies) )
	{
		...
		if( $was_original ) // original is deleted, promote first copy
			$promoted_id = prism_gallery_promote_first_attachment_copy(0, $this_copies);
		
		$uploadpath = wp_upload_dir();
		$file_path  = path_join($uploadpath['basedir'], $_file);
		
		if( prism_gallery_file_is_displayable_image($file_path) ) // if it's an image - regenerate its intermediate sizes
...
add_filter('wp_delete_file', 'prism_gallery_cancel_file_deletion_if_attachment_copies');
 ...
 */
function prism_gallery_delete_all_attachment_copies( $attachment_id )
{
	$copies = get_post_meta($attachment_id, '_has_copies', true);
	
	if( is_array($copies) && ! empty($copies) )
	{
		do_action('prism_gallery_delete_all_attachment_copies', $attachment_id, &$copies);
...
function prism_gallery_handle_deleted_attachment( $post_id )
{
...
add_action('delete_attachment',              'prism_gallery_handle_deleted_attachment');
add_action('prism_gallery_delete_attachment', 'prism_gallery_handle_deleted_attachment');
 ...
 */
function prism_gallery_promote_first_attachment_copy( $attachment_id, $copies = false )
{
	if( false === $copies )
		$copies = get_post_meta($attachment_id, '_has_copies', true);
	...
	if( is_array($copies) && ! empty($copies) )
	{
		...
		do_action('prism_gallery_promote_first_attachment_copy', $attachment_id, &$promoted_id);
