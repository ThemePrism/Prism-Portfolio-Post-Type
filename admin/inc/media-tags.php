...
function prism_gallery_filter_parse_query( $wp_query )
{
	if( isset($wp_query->query_vars[PRISM_GALLERY_MEDIA_TAG_NAME]) )
		$wp_query->query_vars['post_status'] = 'inherit';
}
add_action('parse_query', 'prism_gallery_filter_parse_query');
...
*/
function prism_gallery_media_tags_add_permalink_fields()
{
	...
	add_settings_field(
		...
		create_function('', 'prism_gallery_media_tags_permalink_fields("media_tag_taxonomy_name");'),
		'permalink', 'optional');
	...
	if( ! (isset($mediatags) && is_a($mediatags, 'MediaTags') && defined('MEDIA_TAGS_TAXONOMY')) )
	{
		add_settings_field(
			...
			create_function('', 'prism_gallery_media_tags_permalink_fields("media_tag_taxonomy_slug");'),
			'permalink', 'optional');
	}
}
add_action( 'admin_init', 'prism_gallery_media_tags_add_permalink_fields' );


function prism_gallery_media_tags_permalink_fields( $field )
{
	global $mediatags;
...
function prism_gallery_media_tags_get_taxonomy_slug()
{
	...
	define('PRISM_GALLERY_MEDIA_TAG_SLUG', $tax_slug);
	define('PRISM_GALLERY_MEDIA_TAG_NAME', $tax_name);
}


function prism_gallery_media_tags_update_taxonomy_slug( $old_slug = '', $new_slug = '' )
{
	global $wpdb;
	...
	{
		update_option('prism_gallery_flush_rewrite_rules', 1);
