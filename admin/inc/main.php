 ...
 */
function prism_gallery_list_attachments(&$count_attachments, $post_id, $attachment_order, $checked_attachments, $attachment_orderby = 'menu_order')
{
	...
	if( $attachments )
	{
		...
		foreach( $attachments as $attachment )
		{
			...
			if( "" == $attachment_thumb )
			{
				$attachment_thumb[0] = prism_gallery_https( PRISM_GALLERY_CRYSTAL_URL ). "/" . prism_gallery_get_file_type($attachment->post_mime_type) . ".png";
				$attachment_width    = '';
				$attachment_height   = '';
				$non_image           = ' non_image';
			...
			<li id="image-' . $attachment->ID . '" class="' . implode(" ", $classes) . '" style="width: ' . $_attachment_thumb_width . 'px; height: ' . $attachment_thumb_height . 'px">
				...
				if( "" == $non_image ) :
					$attached_files .= '<a href="' . $large[0] . '" id="in-' . $attachment->ID . '-zoom" class="img_zoom action">
						<img src="' . prism_gallery_https( PRISM_GALLERY_URL ) . '/images/famfamfam_silk/magnifier.png" alt="' . __("Zoom", "file-gallery") . '" title="' . __("Zoom", "file-gallery") . '" />
					</a>';
				endif;
				
				$attached_files .= '<a href="#" id="in-' . $attachment->ID . '-edit" class="img_edit action">
					<img src="' . prism_gallery_https( PRISM_GALLERY_URL ) . '/images/famfamfam_silk/pencil.png" alt="' . __("Edit", "file-gallery") . '" title="' . __("Edit", "file-gallery") . '" />
				</a>
			...
			if( current_user_can('edit_post', $attachment->ID) ) :
				...
				if( "" == $non_image && function_exists('current_theme_supports') && current_theme_supports('post-thumbnails') ) :
					...
					$attached_files .= '<a href="#" class="post_thumb_status action" rel="' . $attachment->ID . '" title="' . $as_featured . '">
							<img src="' . prism_gallery_https( PRISM_GALLERY_URL ) . '/images/famfamfam_silk/star_' . $post_thumb_link . '.png" alt="' . $as_featured . '" />
						</a>';
				...
				$attached_files .= '<a href="#" class="delete_or_detach_link action" rel="' . $attachment->ID . '">
					<img src="' . prism_gallery_https( PRISM_GALLERY_URL ) . '/images/famfamfam_silk/delete.png" alt="' . __("Detach / Delete", "file-gallery") . '" title="' . __("Detach / Delete", "file-gallery") . '" />
				</a>
 ...
 */
function prism_gallery_list_tags( $args = array() )
{
	global $wpdb;
 ...
 */
function prism_gallery_main( $ajax = true )
{
	global $wpdb;
...
add_action('wp_ajax_prism_gallery_load',				'prism_gallery_main');
add_action('wp_ajax_prism_gallery_main_update',		'prism_gallery_main');
add_action('wp_ajax_prism_gallery_main_delete',		'prism_gallery_main');
add_action('wp_ajax_prism_gallery_main_detach',		'prism_gallery_main');
add_action('wp_ajax_prism_gallery_set_post_thumb',	'prism_gallery_main');
add_action('wp_ajax_prism_gallery_unset_post_thumb',	'prism_gallery_main');
?>