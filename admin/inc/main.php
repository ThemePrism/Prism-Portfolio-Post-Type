...
class Prism_Portfolio_Ajax  {

	public static function init(){
		add_action('wp_ajax_prism_gallery_load', array(__CLASS__,'main'));
		add_action('wp_ajax_prism_gallery_main_update',	array(__CLASS__,'main'));
		add_action('wp_ajax_prism_gallery_main_delete',	array(__CLASS__,'main'));
		add_action('wp_ajax_prism_gallery_main_detach',	array(__CLASS__,'main'));
		add_action('wp_ajax_prism_gallery_set_post_thumb', array(__CLASS__,'main'));
		add_action('wp_ajax_prism_gallery_unset_post_thumb', array(__CLASS__,'main'));
		
		add_action('wp_ajax_prism_gallery_save_menu_order', array(__CLASS__,'save_menu'));

		add_action('wp_ajax_prism_gallery_save_toggle_state', array(__CLASS__,'save_toggle_state'));
		add_action('wp_ajax_prism_gallery_save_single_toggle_state', array(__CLASS__,'save_toggle_state'));
		add_action('wp_ajax_prism_gallery_save_acf_toggle_state', array(__CLASS__,'save_toggle_state'));
	 ...
	 */
	function list_attachments(&$count_attachments, $post_id, $attachment_order, $checked_attachments, $attachment_orderby = 'menu_order'){
		...
		if( $attachments )
		{
			...
			// start the list...
			$attached_files = '<ul class="ui-sortable" id="prism_gallery_list">' . "\n";
				...
				<li id="image-' . $attachment->ID . '" class="' . implode(" ", $classes) . '" style="width: ' . $_attachment_thumb_width . 'px; height: ' . $attachment_thumb_height . 'px">
					...
					if( "" == $non_image ) :
						$attached_files .= '<a href="' . $large[0] . '" id="in-' . $attachment->ID . '-zoom" class="img_zoom action">
							<img src="' . Prism_Portfolio::plugin_url() . '/admin/images/famfamfam_silk/magnifier.png" alt="' . __("Zoom", "prism_portfolio") . '" title="' . __("Zoom", "prism_portfolio") . '" />
						</a>';
					endif;
					
					$attached_files .= '<a href="#" id="in-' . $attachment->ID . '-edit" class="img_edit action">
						<img src="' . Prism_Portfolio::plugin_url() . '/admin/images/famfamfam_silk/pencil.png" alt="' . __("Edit", "prism_portfolio") . '" title="' . __("Edit", "prism_portfolio") . '" />
					</a>
					<span class="checker_action action" title="' . __('Click to select, or click and drag to change position', "prism_portfolio") . '">
						<input type="checkbox" id="att-chk-' . $attachment->ID . '" class="checker"' . $checked . ' title="' . __("Click to select", "prism_portfolio") . '" />
					</span>';
				...
				if( current_user_can('edit_post', $attachment->ID) ) :
					...
					if( "" == $non_image && function_exists('current_theme_supports') && current_theme_supports('post-thumbnails') ) :
						...
						if( "set" == $post_thumb_link )
							$as_featured = __("Set as featured image", "prism_portfolio");
						else
							$as_featured = __("Unset as featured image", "prism_portfolio");
					
						$attached_files .= '<a href="#" class="post_thumb_status action" rel="' . $attachment->ID . '" title="' . $as_featured . '">
								<img src="' . Prism_Portfolio::plugin_url() . '/admin/images/famfamfam_silk/star_' . $post_thumb_link . '.png" alt="' . $as_featured . '" />
							</a>';
						
						$attached_files .= '<div id="post_thumb_setter_' . $attachment->ID . '" class="post_thumb_setter">
								' . __("Really set as featured image?", "prism_portfolio") . ' 
								<a href="#" id="post_thumb_set[' . $attachment->ID . ']" class="post_thumb_set">' . __("Continue", "prism_portfolio") . '</a>
								' . __("or", "prism_portfolio") . '
								<a href="#" class="post_thumb_cancel" rel="' . $attachment->ID . '">' . __("Cancel", "prism_portfolio") . '</a>
							</div>';
						
						$attached_files .= '<div id="post_thumb_unsetter_' . $attachment->ID . '" class="post_thumb_unsetter">
								' . __("Really unset as featured image?", "prism_portfolio") . ' 
								<a href="#" id="post_thumb_unset[' . $attachment->ID . ']" class="post_thumb_unset">' . __("Continue", "prism_portfolio") . '</a>
								' . __("or", "prism_portfolio") . '
								<a href="#" class="post_thumb_cancel" rel="' . $attachment->ID . '">' . __("Cancel", "prism_portfolio") . '</a>
							</div>';
					...
					$attached_files .= '<a href="#" class="delete_or_detach_link action" rel="' . $attachment->ID . '">
						<img src="' . Prism_Portfolio::plugin_url() . '/admin/images/famfamfam_silk/delete.png" alt="' . __("Detach / Delete", "prism_portfolio") . '" title="' . __("Detach / Delete", "prism_portfolio") . '" />
					</a>
					...
					if( current_user_can('delete_post', $attachment->ID) ) :
						...
						$attached_files .= '<a href="#" class="do_single_delete" rel="' . $attachment->ID . '">' . __("Delete", "prism_portfolio") . '</a>
							<br />
							' . __("or", "prism_portfolio") . '
							<br />';
						...
						$attached_files .= '<a href="#" class="do_single_detach" rel="' . $attachment->ID . '">' . __("Detach", "prism_portfolio") . '</a>
					</div>
					<div id="detach_attachment_'  . $attachment->ID . '" class="detach_attachment">
						' . __("Really detach?", "prism_portfolio") . ' 
						<a href="#" id="detach[' . $attachment->ID . ']" class="detach">' . __("Continue", "prism_portfolio") . '</a>
						' . __("or", "prism_portfolio") . '
						<a href="#" class="detach_cancel" rel="' . $attachment->ID . '">' . __("Cancel", "prism_portfolio") . '</a>
					</div>';
					
					if( current_user_can('delete_post', $attachment->ID) ) :
					
						$attached_files .= '<div id="del_attachment_' . $attachment->ID . '" class="del_attachment">
							' . __("Really delete?", "prism_portfolio") . ' 
							<a href="#" id="del[' . $attachment->ID . ']" class="delete">' . __("Continue", "prism_portfolio") . '</a>
							' . __("or", "prism_portfolio") . '
							<a href="#" class="delete_cancel" rel="' . $attachment->ID . '">' . __("Cancel", "prism_portfolio") . '</a>
						</div>';
	...
	function list_tags( $args = array() )
	{
		...
		$media_tags = $wpdb->get_results(		 
			"SELECT * FROM $wpdb->terms 
			 ...
			 WHERE $wpdb->term_taxonomy.taxonomy = '" . PRISM_GALLERY_MEDIA_TAG_NAME . "'
		...
		if( !empty($media_tags) )
		{
			...
			else // html
			{
				if( $link )
				{
					...
					foreach( $media_tags as $tag )
					{						
						$list[] = '<a href="' . prism_gallery_https( get_bloginfo("url") ) . $fs . $media_tag_slug . $ss . $tag->slug . $ts . '" class="fg_insert_tag" name="' . $tag->slug . '">' . $tag->name . '</a>';
					}
				}
				else
				{
	...
	function main( $ajax = true )
	{
		...
		$count_attachments    = 0;
		$hide_form            = '';
		$sizes                = prism_gallery_get_intermediate_image_sizes();
		...
		if( 'prism_gallery_main_delete' == $action )
		{
			...
			define("PRISM_GALLERY_SKIP_DELETE_CANCEL", true);
			...
			foreach( $copies as $copy )
			{
				...
				{
					prism_gallery_delete_attachment( $copy );
			...
			foreach( $originals as $original )
			{
				...
				{
					prism_gallery_delete_all_attachment_copies( $original );
				...
				{
					prism_gallery_promote_first_attachment_copy( $original );
					prism_gallery_delete_attachment( $original );
			...
			if( empty($fully_deleted) && empty($partially_deleted) )
				$output = __("No attachments were deleted (capabilities?)", "prism_portfolio");
			else
				$output = __("Attachment(s) deleted", "prism_portfolio");
		}
		elseif( "prism_gallery_main_detach" == $action )
		{
			...
			if( empty($detach_errors) )
				$output = __("Attachment(s) detached", "prism_portfolio");
			else
				$output = __("Error detaching attachment(s)", "prism_portfolio");
		}
		elseif( "prism_gallery_main_update" == $action )
		{
			...
			$get_old_media_tags = wp_get_object_terms((int) $_POST['attachment_id'], PRISM_GALLERY_MEDIA_TAG_NAME);
			...
			if( "" != $_POST['tax_input'] && $old_media_tags != $_POST['tax_input'] )
			{
				...
				$media_tags_result = wp_set_object_terms( $attachment_id, $tax_input, PRISM_GALLERY_MEDIA_TAG_NAME );
			}
			elseif( "" == $_POST['tax_input'] )
			{
				$media_tags_result = wp_set_object_terms( $attachment_id, NULL, PRISM_GALLERY_MEDIA_TAG_NAME );
			}
			
			// check if there were any changes
			$old_attachment_data = get_object_vars( get_post($attachment_id) );
			
			if( prism_gallery_file_is_displayable_image(  get_attached_file($attachment_id) ) )
				...
				is_array($tax_input) )
			{
				if( 0 !== wp_update_post($attachment_data) )
				{
					...
					$output = __("Attachment data updated", "prism_portfolio");
				}
				else
				{
					$output = __("Error updating attachment data!", "prism_portfolio");
				}
			}
			else
			{
				$output = __("No change.", "prism_portfolio");
			}
		}
		elseif( "prism_gallery_set_post_thumb" == $action )
		{
			update_post_meta($post_id, "_thumbnail_id", $attachment_ids[0]);
			exit(_wp_post_thumbnail_html($attachment_ids[0]));
		}
		elseif( "prism_gallery_unset_post_thumb" == $action )
		{
			exit();
		}
	...
	function save_menu()
	{
		...
		{
			echo __('Attachment order saved successfully.', "prism_portfolio");
		}
		else
		{
			$error = __('Database error! Function: prism_gallery_save_menu', "prism_portfolio");
	...
	function save_toggle_state()
	{
		...
		switch( $_POST['action'] )
		{
			case 'prism_gallery_save_single_toggle_state' :
				$opt = 'insert_single_options_state';
				break;
			case 'prism_gallery_save_acf_toggle_state' :
				$opt = 'acf_state';
				break;
			default : 
				break;
