...
class Prism_Portfolio_Post_Type {
	...
	public static function init() { 
		...
		add_filter( 'post_type_link', array(__CLASS__,'filter_post_link'), 10, 4 );
		...
		add_action('right_now_content_table_end', array(__CLASS__,'add_counts'));
	...
	function icons() { ?>
		<style type="text/css" media="screen">
			#menu-posts-prism_portfolio .wp-menu-image {
				background: url(<?php echo Prism_Portfolio::plugin_url() . '/admin/images/portfolio-icon.png'; ?>) no-repeat 6px 6px !important;
			}
			#menu-posts-prism_portfolio:hover .wp-menu-image, #menu-posts-prism_portfolio.wp-has-current-submenu .wp-menu-image {
				background-position:6px -16px !important;
			}
			#icon-edit.icon32-posts-prism_portfolio {background: url(<?php echo Prism_Portfolio::plugin_url() .'/admin/images/portfolio-32x32.png'; ?>) no-repeat;}
		</style>
	<?php }
	...
	function add_counts() {
			if ( ! post_type_exists( Prism_Portfolio::$post_type ) ) {
				 return;
			}

			$num_posts = wp_count_posts( Prism_Portfolio::$post_type );
			$num = number_format_i18n( $num_posts->publish );
			$text = _n( 'Portfolio Item', 'Portfolio Items', intval($num_posts->publish) );
			if ( current_user_can( 'edit_posts' ) ) {
				$num = sprintf('<a href="edit.php?post_type=%s">%d</a>',Prism_Portfolio::$post_type,$num);
				$text = sprintf('<a href="edit.php?post_type=%1$s">%2$s</a>',Prism_Portfolio::$post_type,$text);
			}
			...
			if ($num_posts->pending > 0) {
				...
				if ( current_user_can( 'edit_posts' ) ) {
					$num = sprintf('<a href="edit.php?post_status=pending&post_type=%s">%d</a>', Prism_Portfolio::$post_type ,$num );
					$text = sprintf('<a href="edit.php?post_status=pending&post_type=%1$s">%2$s</a>', Prism_Portfolio::$post_type, $text );
				}
				echo '<td class="first b b-portfolio">' . $num . '</td>';
				echo '<td class="t portfolio">' . $text . '</td>';
