<?php
/**
 * Prism Post Type Metabox
 * 
 * The Prism Post Type Metaboxes class creates, saves and validates the metaboxes
 *
...
class Prism_Metaboxes extends Prism_Portfolio {
	...
	*/
	function save_taxonomy_data($post_id,$post) {  
		...
		if (isset($_POST['prism_portfolio_featured_tax']) ) { 
			$file = esc_attr($_POST['prism_portfolio_featured_tax']);
			$allowed = array ( 'featured','normal','archived');
			if ($file and in_array($file,$allowed)) {
				wp_set_object_terms( $post_id, $file, 'prism_portfolio_featured' );
			} else { 
...
// Add the Documents Metabox
	static function gallery_metabox() {
		...
		add_meta_box('gallery_metabox', __('Gallery',self::TEXT_DOMAIN), array(__CLASS__,'gallery_metabox_callback'), self::$post_type, 'normal', 'high');
	}
	...
	static function plupload_action() {
		...
		// you can use WP's wp_handle_upload() function:
	 	...
	 	//Adds file as attachment to WordPress
  		$file['attachment_id'] = wp_insert_attachment( array(
		...
		if ($file_type['type']) {
			$attach_data = wp_generate_attachment_metadata( $file['attachment_id'], $file['file'] );
